...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('status', sa.Enum('DRAFT', 'PENDING', 'PAYMENT_SUBMITTED', 'PAYMENT_VERIFIED', 'CONFIRMED', 'CANCELLED_BY_USER', 'CANCELLED_BY_ADMIN', 'COMPLETED', name='registrationstatus'), nullable=False, server_default='PENDING'),
        sa.Column('created_at', sa.TIMESTAMP(), nullable=False, server_default=sa.text('CURRENT_TIMESTAMP')),
        sa.Column('updated_at', sa.TIMESTAMP(), nullable=False, server_default=sa.text('CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP')),
        sa.ForeignKeyConstraint(['yatra_id'], ['yatras.id'], ondelete='CASCADE'),
